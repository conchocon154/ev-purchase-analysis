...
This submission scores **0.94087** public ROC-AUC — **rank 398 of 471**, below
the median of 0.94184. It is `HistGradientBoostingClassifier` on the thirteen
raw features, no feature engineering and no hyperparameter search, chosen over
logistic regression (0.93809) and a rank-average blend (0.94040) by out-of-fold
score. Cross-validation predicted 0.94133 and the leaderboard returned 0.94087,
so the validation was honest even though the model is plain.

Worth noting what that rank means here: the gap from rank 1 (0.94644) to the
25th percentile (0.94138) is **0.005 AUC**. The competitive range is narrower
than the difference between "gender is significant" and "gender is useless" — a
distinction that decides whether a marketing budget is wasted. The leaderboard
and the analysis are answering different questions, and this notebook was built
for the second one.
